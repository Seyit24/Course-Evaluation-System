...
function DasherButton(button) {
  ...
  this.dashButton.on("detected", function() {
    ...
    if(button.name == "Schwarzkopf Button" || button.name == "PowerPoint Button" || button.name == "Nerf Button"){
        bewertung = "schlecht";
	bGedrücktSchlecht = bGedrücktSchlecht + 1;
	ergebnis = bGedrücktSchlecht / pressed *100 ;
	doLog(ergebnis + " Prozent finden die Vorlesung schlecht");
	//doLog("Anzahl schlechte Dash-Buttons gedrückt: " +  bGedrücktSchlecht);
	pressed++;
    } else {
        bewertung = "gut";
	bGedrücktGut = bGedrücktGut + 1;
	ergebnis = bGedrücktGut / pressed *100 ;
	doLog(ergebnis + " Prozent finden die Vorlesung gut");
	//doLog("Anzahl gute Dash-Buttons gedrückt: " +  bGedrücktGut);
	pressed++;
    }
    //doLog("Anzahl Bewertungen: " + pressed);
    //doLog(ergebnis);
